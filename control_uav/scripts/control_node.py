...
class gnc_api:
    global _FLAG_TAKEOFF, _T_UPDATE_POS
    ...
    def __init__(self):
        ...
        '''Add img signal'''
        self.img_signal = rospy.Subscriber(
                                          name="{}img_signal".format(self.ns),
                                          data_class=Bool,
                                          queue_size=10,
                                          callback=self.move_from_img)
    ...
    def move_from_img(self, message):
        global _FLAG_TAKEOFF
        global _T_UPDATE_POS
        cur_pose = self.get_current_location()
        if(_FLAG_TAKEOFF and message.data and ((time.time() - _T_UPDATE_POS) > 0.100)): 
            self.set_destination(0.5+cur_pose.x, 0, 5,0)
            _T_UPDATE_POS = time.time()
        if(_FLAG_TAKEOFF and not(message.data)):
            print('Desination has reached')
    ...
    def set_destination_no_log(self, x, y, z, psi):
        """This function is used to command the drone to fly to a waypoint. These waypoints should be specified in the local reference frame. This is typically defined from the location the drone is launched. Psi is counter clockwise rotation following the drone's reference frame defined by the x axis through the right side of the drone with the y axis through the front of the drone.
...
def main():
  ...
  controller.wait4connect()
  # Wait for the mode to be switched.
  controller.wait4start()
  # Create local reference frame.
  controller.initialize_local_frame()
